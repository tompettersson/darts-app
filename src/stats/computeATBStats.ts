...
import { applyATBEvents, getSequence, DEFAULT_ATB_CONFIG, type ATBEvent, type ATBTurnAddedEvent } from '../dartsAroundTheBlock'
...
// =====================================================
// PIRATENMODUS - Spezielle Statistiken
// =====================================================

/**
 * Berechnet detaillierte Statistiken für Piratenmodus
 */
function computeATBDetailedStatsPirate(
  match: ATBStoredMatch,
  state: any
): ATBDetailedStats[] {
  const sequence = state.match.extendedSequence
    ? state.match.extendedSequence.map((t: any) => t.number)
    : state.match.sequence || []

  const results: ATBDetailedStats[] = []

  // Im Piratenmodus: Sammle Round-Events um zu wissen welches Feld wann aktiv war
  const roundEvents = match.events.filter((e: any) => e.type === 'ATBPirateRoundFinished')

  for (const player of state.match.players) {
    const pid = player.playerId

    // Sammle alle Turn-Events für diesen Spieler (in chronologischer Reihenfolge)
    const turnEvents = match.events.filter(
    ...
    // Für jeden Turn des Spielers
    for (let turnIdx = 0; turnIdx < turnEvents.length; turnIdx++) {
      const turn = turnEvents[turnIdx]

      // Finde das aktuelle Feld für diesen Turn:
      // Suche das nächste RoundFinished-Event nach diesem Turn's Timestamp
      let targetField: string | number | null = null

      for (const roundEvent of roundEvents) {
        const roundTime = new Date((roundEvent as any).ts).getTime()
        const turnTime = new Date(turn.ts).getTime()

        if (roundTime >= turnTime) {
          // Dies ist das Feld, das dieser Turn betrifft
          targetField = (roundEvent as any).fieldNumber
          break
        }
      }
      ...
      // Zähle die Darts in diesem Turn
      let turnHits = 0
      let turnMisses = 0
      ...
      // Zähle Attempts (ein Turn = ein Versuch auf diesem Feld)
      statsPerField[fieldKey].attempts++
      if (turnHits > 0) {
        fieldsCompleted++
      }

      // Perfect Turn (alle 3 Darts treffer)
    ...
    // Berechne Finals
    const avgDartsPerField = fieldsCompleted > 0 ? totalDarts / fieldsCompleted : 0
    const firstDartHitRate = fieldsCompleted > 0 ? (firstDartHits / fieldsCompleted) * 100 : 0
    ...
    results.push({
      ...
      longestMissSeries: 0,
      problematicFields: [],
      firstDartStreak: longestStreak,
      perfectTurns,
      comparison1to10: 0,
      comparison11toBull: 0,
      isWinner: match.winnerId === pid,
    })
  }
...
export function computeATBDetailedStats(
  ...
  // Prüfe ob Piratenmodus
  const isPirate = (match.config ?? DEFAULT_ATB_CONFIG).gameMode === 'pirate'
  if (isPirate && legIndex === undefined) {
    return computeATBDetailedStatsPirate(match, state)
  }
